...
context â€” the rules below are identical either way, but the context guarantees are not.
...
`git add ... && git commit ...` fails *silently as far as your reasoning is concerned* â€” you
...
to the next task. Diagnose once â€” usually a failed `git add`, an unexpected shell, or nothing
to commit â€” then either fix it and retry, or stop and return `status: blocked`.
...
**Track commit hash** for Summary reporting â€” the real one, from `git log -1`.
