---
name: gsd-subagent-delegation
description: Canonical protocol for delegating GSD work to native Antigravity subagents â€” when to delegate, how to invoke, workspace isolation modes, and the inline fallback for older IDE versions
---
...
Delegation is not an optimization in GSD â€” it is the mechanism that makes the quality
...
probe by calling it â€” a failed call wastes a turn and confuses the user.
...
**Plan requirements:** none beyond 2.0. Plain subagents are not plan-gated â€” only
...
Everything else is delegable. When in doubt, delegate â€” the cost of an extra subagent is
...
definition. The tool's exact parameter schema is supplied by the IDE â€” follow it. This
...
2. **Paths, not contents** â€” `.gsd/phases/2/1-PLAN.md`, never the pasted plan
3. **Constraints from STATE.md** that bound the work â€” decisions already made, known blockers
4. **The return contract** â€” restate that you want the compact block, nothing more
...
**`tools:` defaults to an empty list â€” it does not inherit the parent's toolset.** A subagent
...
than fail â€” the worst possible failure mode, because it looks like slow work.
...
| Mode | Use when |
|------|----------|
| `inherit` | Sequential work â€” one subagent touching the workspace at a time |
| `branch` | Two or more subagents writing files concurrently (isolated git worktree) |
| `share` | Read-only concurrent work â€” research, mapping, verification |
...
a workspace corrupt each other's commits â€” this is not a theoretical risk.

---

## Handling Results

A subagent returns its compact block. That block is what enters your context â€” nothing else.
...
- **A dead subagent is not a completed one.** If a subagent returns nothing or dies, report
  it as a failure and stop. Do not silently redo its work inline â€” that is precisely the
...
- **A subagent that returns file contents has failed, however complete it sounds.** The
  ...
  find out which capability it lacked, and fix the definition â€” do not paste the content
  onward, or the delegation cost you tokens and bought you nothing.

Depth is capped at 10 levels of nesting. GSD never needs more than 2 â€” orchestrator to
...
```
âš ï¸  Subagent delegation unavailable (requires Antigravity 2.0+)
    Running inline â€” context will fill faster.
