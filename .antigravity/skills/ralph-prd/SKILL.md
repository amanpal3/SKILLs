...
2. Ask **5â€“10** clarifying questions (with lettered options), in batches of up to 5 at a time (ask 5, wait for answers, then ask the next batch if needed).
...
- `prd-<short-slug>.json` where `<short-slug>` is 1â€“3 meaningful words (avoid filler like â€œi want toâ€).
